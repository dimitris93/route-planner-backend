...
ShortestPath::ShortestPath(int s, int ignore_node,
                           float uv_cost,
                           ...
                           const vector<vector<AdjListElement>> &adjacency_lists)
{
    ...
    // Init variables
    float    max_path_cost = numeric_limits<float>::min();
    for (int i             = targets.size() - 1; i >= 0; --i)
    {
        const int t = targets[i].v;
        if (s == t)
        {
            targets.erase(targets.begin() + i);
            continue;
        }
        const float w = targets[i].w;
        if (w > max_path_cost)
            max_path_cost = w;
    }
    max_path_cost += uv_cost;
    ...
    while (!pq.empty())
    {
        ...
        if (dist_to_u > max_path_cost)
            break;
        // iff found distance(u, w) <= weight(uv) + weight(vw)
        // then add_shortcut = false
        for (int i = 0; i < targets.size(); ++i)
        {
            if (u == targets[i].v)
            {
                if (dist_to_u <= uv_cost + targets[i].w)
                {
                    targets.erase(targets.begin() + i);
                }
                break;
