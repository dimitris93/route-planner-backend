...
## Future work
 ...
 - Right now, we use `connected components labeling` to determine if 2 given coordinates from the user are connected and if they are not, then we respond with a `path-does-not-exist` message. This occurs if the user gives coordinates between 2 different isolated graphs. However, a more elegant solution is to keep searching (info [here](#finding-nearest-edges-from-users-given-coordinates) to explain how we do this search) until we find 2 nodes that belong to the same connected components. This way we would be able to calculate a shortest path, even though it would not include the road that is the nearest to the user's given coordinate. This is not trivial to implement, and it requires the use of a few tuning parameters. We have to remember that our routing engine supports in-between nodes as well. The user could give a A->B->C query where the 3 coordinates are close to 3 different edges that belong to 3 different connected components. In this case we would have to determine which alternative edges we have to select, so that the query does not result in `path-does-not-exist`. To do this we would need to take into account the size of each of the 3 connected components (we prefer edges that belong to larger components, instead of tiny ones like parking or a blocked road) as well as the distance between the alternative edge and the nearest edge to the user's given coordinate. We would also need to consider the fact that the search for an alternative edge could be costly. If we had the pedestrial graph (foot graph) and the user gave a query from america to africa, then it would be a waste of time to search for alternate edges to make this query happen, because those alternate edges would be far off from the user's given coordinate. That means that out search for alternate edges has to have a proper stopping condition. That stopping condition would prevent spending time to try and solve illogical queries, for example from america to africa.
